    ...
    return {
      ...
      fireEvents: function (V, L) {
        ...
        var N = YAHOO.util.Event.getPageX(V),
          ...
          D = {},
          Q = [],
          c = { outEvts: [], overEvts: [], dropEvts: [], enterEvts: [] };
...
function jsDropdown(opts, selected, cssClass, optClasses, titles) {
  ...
  this.expand = function () {
    ...
    if (y + height > Dom.getViewportHeight() + Dom.getDocumentScrollTop()) {
      var newY = Math.max(
        180,
        y - height - sCurrentSelected.offsetHeight,
...
function replaceSelect(mirroredSelect) {
  ...
  YAHOO.util.Event.addListener(
    replacement.getCurrentSelected(),
    "click",
    this.focusMirrored,
    this,
    true
  ...
  mirroredSelect.onfocus = function () {
    ...
    if (replacement.getSelectContainer().firstChild) {
      YAHOO.util.Dom.addClass(
        replacement.getSelectContainer().firstChild,
        "hover"
      );
    }
  };
  ...
  mirroredSelect.parentNode.insertBefore(
    replacement.getSelectContainer(),
    mirroredSelect
  );
}
  ...
  for (var i = 0; i < obj.length; i++) {
    ...
    if (obj[i].checked) {
      document.getElementById(imageName).src =
        "skin/interface/radio_circle_checked.gif";
    } else {
      document.getElementById(imageName).src =
        "skin/interface/radio_circle_off.gif";
    }
  }
}
...
function getTimestring(
  ...
  var loca = [];
  loca["day"] = showunits
    ? LocalizationStrings["timeunits"]["short"]["day"]
    : "";
  loca["hour"] = showunits
    ? LocalizationStrings["timeunits"]["short"]["hour"]
    : "";
  loca["minute"] = showunits
    ? LocalizationStrings["timeunits"]["short"]["minute"]
    : "";
  loca["second"] = showunits
    ? LocalizationStrings["timeunits"]["short"]["second"]
    : "";
...
function getResourceCounter(config) {
  ...
  resCounter.subscribe("update", function () {
    this.currentRes =
      this.startRes +
      this.production * Math.floor((this.currenttime - this.startdate) / 1000);
    this.checkBounds();
    for (var i = this.spendings.length - 1; i >= 0; --i) {
      this.currentRes =
        this.currentRes -
        (this.spendings[i]["amount"] *
          Math.floor(
            (this.currenttime - this.startdate) /
              1000 /
              this.spendings[i]["tickInterval"]
          ) *
          this.spendings[i]["tickInterval"]) /
          3600;
    }
...
function getProgressBar(config) {
  ...
  pbar.subscribe("update", function () {
    this.progress = Math.floor(
      ((this.currenttime - this.startdate) / this.duration) * 100
    );
...
(function () {
  ...
  proto.initAttributes = function (attr) {
    ...
    this.setAttributeConfig("dataSrc", { value: attr.dataSrc });
      ...
      method: function (value) {
        if (value) {
          ...
          if (this.get("dataSrc")) {
            if (
              !this._loading &&
              !(this.get("dataLoaded") && this.get("cacheData"))
            ) {
...
function Drag() {
  ...
  this.startDetection = function (ev) {
    ...
    this.dragStartOffset = [
      this.dragstartCoords[0] - tElXY[0],
      this.dragstartCoords[1] - tElXY[1],
    ];
  ...
  this.detectDrag = function (ev) {
    ...
    if (new Date().getTime() > this.dragstartTime + 100) {
      ...
      Math.abs(Event.getPageX(ev) - this.dragstartCoords[0]) > 8 &&
      Math.abs(Event.getPageY(ev) - this.dragstartCoords[0]) > 8
    ) {
      this.dragactive = true;
    }
    if (this.dragactive) {
  ...
  this.endDrag = function (ev) {
    ...
    scheduler.schedule(
      ...
      [document, "click", this.stopClicks],
      Event
    );
...
function Draggable(el) {
  ...
  this.whileDrag = function (coords) {
    delta = [
      d.coords[0] - d.dragstartCoords[0],
      d.coords[1] - d.dragstartCoords[1],
    ];
...
function Slider(el, config) {
  ...
  input = Dom.get(input);
  var valueEl =
    typeof config.valueEl == "undefined"
      ? Dom.getFirstChildBy(container, function (el) {
  ...
  if (typeof config.hiddenValues != "undefined") {
    ...
    var hiddenValueEl =
      typeof config.hiddenValueEl == "undefined"
        ? Dom.getFirstChildBy(container, function (el) {
  ...
  this.setConstraints = function (b) {
    boundaries =
      typeof b == "undefined"
        ? [
            convertStylePx(currentStyle.paddingTop),
            convertStylePx(currentStyle.paddingLeft) +
              convertStylePx(currentStyle.width) -
              thumb.offsetWidth,
            convertStylePx(currentStyle.paddingTop) +
              convertStylePx(currentStyle.height) -
              thumb.offsetHeight,
            convertStylePx(currentStyle.paddingLeft),
          ]
        : b;
  ...
  this.clickContainer = function (ev) {
    if (Event.getTarget(ev) != thumb) {
      ...
      this.setPosition([
        evPos[0] - conPos[0] - thumb.offsetWidth / 2,
        evPos[1] - conPos[1] - thumb.offsetWidth / 2,
      ]);
    }
  };
  ...
  this.checkHidden = function () {
    if (this.value >= hiddenRange[1] && hidden) {
      ...
      if (dir == "ltr") {
        hiddenValueEl.style.clip =
          "rect(0px auto auto " +
          Math.round(
            (1 - hiddenValues / absRange) * pixelScale +
              boundaries[3] +
              thumb.offsetWidth / 2
          ) +
          "px)";
        ...
        hiddenValueEl.style.clip =
          "rect(0px " +
          Math.round(
            (hiddenValues / absRange) * pixelScale +
              boundaries[3] +
              thumb.offsetWidth / 2
          ) +
          "px auto 0px)";
  ...
  if (dir == "ltr") {
    this.valueByPosition = function (p) {
      return absRange > 0
        ? Math.round((p[0] - boundaries[3]) / (pixelScale / absRange)) +
            Math.min(this.range[0], this.range[1])
        : this.range[0];
    };
  } else if (dir == "rtl") {
    this.valueByPosition = function (p) {
  ...
  if (dir == "ltr") {
    this.positionByValue = function (v) {
      ...
      if (absRange > 0) {
        p[0] = Math.round(
          (v - Math.min(this.range[0], this.range[1])) *
            (pixelScale / absRange) +
            boundaries[3]
        );
        p[1] = boundaries[0];
    ...
    this.positionByValue = function (v) {
      ...
      if (absRange > 0) {
        p[0] = Math.round(
          (v - Math.min(this.range[0], this.range[1]) - absRange) *
            -1 *
            (pixelScale / absRange) +
            boundaries[3]
        );
  ...
  this.keepHiddenValueBounds = function (v) {
    return v > hiddenRange[1] + hiddenValues
      ? hiddenRange[1] + hiddenValues
      : v < hiddenRange[0]
      ? hiddenRange[0]
      : v;
  };
  ...
  this.dragPosition = function (p) {
    ...
    position[0] = Math.round(
      Math.round((position[0] - boundaries[3]) / (pixelScale / absRange)) *
        (pixelScale / absRange) +
        boundaries[3]
    );
  ...
  if (dir == "ltr") {
    this.draw = function () {
      ...
      valueEl.style.clip =
        "rect(0px " + (position[0] + thumb.offsetWidth / 2) + "px auto 0px)";
    ...
    this.draw = function () {
      ...
      thumb.style.top = position[1] + "px";
      valueEl.style.clip =
        "rect(0px auto auto " + (position[0] + thumb.offsetWidth / 2) + "px)";
      container.title = +this.value;
    };
  }
  if (typeof hiddenValues != "undefined") this.checkHidden();
...
function transportController(config) {
  ...
  var spaceReserved =
    typeof config.spaceReserved == "undefined" ? 0 : config.spaceReserved;
  var that = this;
  var rI = [];
  ...
  this.registerInput = function (input, weight) {
    if (!weight) weight = 1;
    rI.push({ input: input, weight: weight, oldMax: input.range[1] });
  ...
  this.setTransporters = function (n) {
    ...
    if (this.remainingCapacity < 0) {
      ...
      for (var i = rI.length - 1; i >= 0; i--) {
        var multiplier =
          tC / capacityPerTransport / (uC / capacityPerTransport);
  ...
  this.realignInput = function (v) {
    ...
    for (var i = rI.length - 1; i >= 0; i--) {
      rI[i].input.setRange([
        0,
        Math.min(rI[i].oldMax, this.remainingCapacity + rI[i].input.value),
      ]);
    }
  };
}
...
function journeyDurationController() {
  ...
  this.registerInput = function (input, duration) {
    ...
    input.subscribe("change", function (v) {
    ...
    for (i = 0; i < handleEls.length; i++) {
      if (handleEls[i] == this.hoverEl) {
        ...
        if (this.status) {
          Dom.removeClass(
            handleEls[i],
            handleBaseClasses[i] + "_extended_hover"
          );
          Dom.addClass(handleEls[i], handleBaseClasses[i] + "_extended");
  ...
  YAHOO.extend(B, F, {
            ...
            if (R != "visible" || R === "") {
              ...
              for (a = 0; a < S; a++) {
                ...
                if (
                  a === 0 &&
                  !f(
                    O.animateInCompleteEvent,
                    this.showEvent.fire,
                    this.showEvent
                  )
                ) {
                  O.animateInCompleteEvent.subscribe(
                    this.showEvent.fire,
                    this.showEvent,
                    true
                  );
          ...
          if (R == "visible") {
            ...
            for (Z = 0; Z < S; Z++) {
              ...
              if (
                Z === 0 &&
                !f(
                  c.animateOutCompleteEvent,
                  this.hideEvent.fire,
                  this.hideEvent
                )
              ) {
                c.animateOutCompleteEvent.subscribe(
                  this.hideEvent.fire,
                  this.hideEvent,
                  true
                );
        ...
        if (P) {
          if (typeof P == "string") {
            this.cfg.setProperty(
              "context",
              [document.getElementById(P), N, T, Q],
              true
            );
          }
          if (N && T) {
    ...
    getConstrainedX: function (U) {
      var R = this,
        ...
        O,
        h = U,
        T = { tltr: true, blbr: true, brbl: true, trtl: true };
      var Y = function () {
        var j;
    ...
    getConstrainedY: function (Y) {
      var V = this,
        ...
        R,
        N = Y,
        X = { trbr: true, tlbl: true, bltl: true, brtr: true };
      var S = function () {
        var j;
...
(function () {
  ...
  var E = YAHOO.lang,
    ...
    Q,
    C,
    D = { SHOW_MASK: "showMask", HIDE_MASK: "hideMask", DRAG: "drag" },
    N = {
      CLOSE: {
        key: "close",
        value: true,
  ...
  YAHOO.extend(A, YAHOO.widget.Panel, {
            ...
            if (F.isFunction(T.handler)) {
              ...
              if (F.isObject(T.handler) && F.isFunction(T.handler.fn)) {
                B.on(
                  ...
                  "click",
                  T.handler.fn,
                  !F.isUndefined(T.handler.obj) ? T.handler.obj : this,
    ...
    getData: function () {
      ...
      function Q(c) {
        ...
        return (
          (b == "INPUT" || b == "TEXTAREA" || b == "SELECT") && c.name == M
        );
      }
      if (Y) {
        K = Y.elements;
...
function resourceStack(config) {
  ...
  this.setIcons = function (numIcons) {
    ...
    if (numIcons > (containerwidth - icon.width - 1) / 2)
      numIcons = (containerwidth - icon.width - 1) / 2;
      ...
      if (overlap <= 0) {
        ...
        if (pos % 1 != 0) {
          var roundingInterval =
            (numIcons - 1) / Math.round((pos % 1) * (numIcons - 1));
...
IKARIAM.helpers = {};
IKARIAM.helpers.shortenValue = function (value, to) {
  var steps = { k: 3, m: 6 };
...
IKARIAM.helpers.locaNumberFormat = function (number, to) {
...
IKARIAM.helpers.number_format = function (
  number,
  decimals,
  dec_point,
  thousands_sep
...
var shortenValue = IKARIAM.helpers.shortenValue;
var locaNumberFormat = IKARIAM.helpers.locaNumberFormat;
var number_format = IKARIAM.helpers.number_format;
  ...
  YAHOO.extend(B, YAHOO.util.Element, {
    ...
    _createHandles: function () {
      ...
      for (var F = 0; F < G.length; F++) {
        ...
        this._handles[G[F]].className =
          this.CSS_HANDLE + " " + this.CSS_HANDLE + "-" + G[F];
        ...
        this._dds[G[F]] = new YAHOO.util.DragDrop(
          this._handles[G[F]],
          this.get("id") + "-handle-" + G,
          { useShim: this.get("useShim") }
        );
    ...
    _handleMouseUp: function (F) {
      ...
      E.removeClass(
        this._handles[this._currentHandle],
        this.CSS_HANDLE + "-" + this._currentHandle + "-active"
      );
      E.removeClass(
        this._handles[this._currentHandle],
        this.CSS_HANDLE + "-active"
      );
      if (this.browser.ie) {
        E.addClass(this._wrap, this.CSS_RESIZE);
      }
    ...
    _updateStatus: function (K, G, J, F) {
      if (this._resizeEvent && !C.isString(this._resizeEvent)) {
        ...
        this._status.innerHTML =
          ...
          H +
          "</em>";
        E.setXY(this._status, [
          A.getPageX(this._resizeEvent) + 12,
          A.getPageY(this._resizeEvent) + 12,
        ]);
      }
    },
    lock: function (D) {
    ...
    reset: function () {
      this.resize(
        null,
        this._cache.start.height,
        this._cache.start.width,
        this._cache.start.top,
        this._cache.start.left,
        true
      );
      return this;
    },
    resize: function (M, J, P, Q, H, F, K) {
      ...
      if (I) {
        if (YAHOO.util.Anim) {
          var D = new YAHOO.util.Anim(
            G,
            {
              height: { to: this._cache.height },
              width: { to: this._cache.width },
            },
            this.get("animateDuration"),
            this.get("animateEasing")
          );
          if (this._positioned) {
            if (Q) {
